...
set
  ...
  billing_updated_at = coalesce(billing_updated_at, updated_at, timezone('utc', now()))
where billing_status is null;

alter table public.user_profiles
  alter column billing_status set default 'inactive';

create index if not exists user_profiles_paddle_transaction_id_idx
  on public.user_profiles (paddle_transaction_id);

create index if not exists user_profiles_paddle_subscription_id_idx
  on public.user_profiles (paddle_subscription_id);
