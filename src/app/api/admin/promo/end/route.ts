...
export async function POST() {
    ...
    for (const user of promoUsers) {
      ...
      try {
        await transporter.sendMail({
          ...
          replyTo: "support@policypack.org",
          to: user.email,
          subject: "PolicyPack Ã¢â‚¬â€ Launch Offer Update",
          ...
          html: `<div style="font-family:Inter,Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;">
            ...
            <a href="mailto:support@policypack.org">support@policypack.org</a></p>
          </div>`,
        });
        notifiedCount++;
      } catch (err) {
        notificationErrors.push(`${user.email}: ${err instanceof Error ? err.message : "unknown"}`);
      }
    }
  }
  ...
  const report = {
    ...
    notificationErrors,
    durationMs,
    promoUsersSnapshot: promoUsers.map((u) => ({ email: u.email, createdAt: u.createdAt })),
  };
  ...
  await sendAdminNotification({
    kind: "payment",
    subject: "PolicyPack Ã¢â‚¬â€ Promo Campaign Ended",
